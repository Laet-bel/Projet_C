...
/// <summary>
/// R�cup�re l'histogramme d'une image en nuance de gris
/// </summary>
...
/// </summary>
/// <param name="image"></param>
/// <param name="table">Chemin d'acc�s du fichier</param>
/// <returns></returns>
...
/// <summary>
/// Prend une image lab�lis� (pas coloris�) et r�cup�re diff�rentes signature pour nos composantes connexes
/// </summary>
/// <param name="image"></param>
...
/// <summary>
/// Trouver le pixel minimum de SE appliquer � la coordonn�e sur l'image
/// </summary>
/// <param name="img"> Image initiale </param>
/// <param name="SE"> el�ment structurant </param>
/// <param name="i"> coordonn�e x</param>
/// <param name="j"> coordonn�e y </param>
/// <returns></returns>
...
float* Image_In(char** imagePaths, char** veriteTerrainPaths, ELEMENT_STRUCTURANT se, int nb_it)
{
	...
	// N'oubliez pas de lib�rer la m�moire
	free(tableau_IOU);
	free(tableau_Moy);
...
//Trier les blobs qui n'appartiennent pas � l'image de v�rit�e
IMAGE recuperation_blobs_communs(IMAGE image_verite, IMAGE image_traitee)
{
	//Lab�lisation des images v�rit� et trait�e
	int nb_blobs_traitee;
	IMAGE labelise_image_traitee = labelImage(image_traitee, &nb_blobs_traitee);

	//R�cup�ration des caract�ristiques de la composante connexe de l'image traitee
	SIGNATURE_COMPOSANTE_CONNEXE* blobs_traitee = signaturesImage(labelise_image_traitee, nb_blobs_traitee);

	//Cr�ation d'une LUT pou d�terminers quels blobs deviendrons 0 ou 255
	int* LUT = (int*)calloc(256, sizeof(int));

	//Parcour de l'image v�rit�e
	for (int i = 0; i < image_verite.Nblig; i++)
	{
		for (int j = 0; j < image_verite.Nbcol; j++)
		{
			//Pour chaque pixel > 0 on teste si un blob de l'image trait�e appartient � un blob de l'image v�rit�e
	...
	// J'applique ma LUT pour ne garder que les blobs appartenant � mon image v�rit�e. 
...
POINT* imageVersPoints(IMAGE image, int* n, char axe) {
	...
	if (axe == "x") {// Fixe le x � i et le y � j
		for (int j = 0; j < image.Nbcol; j++) {
			for (int i = 0; i < image.Nblig; i++) {
